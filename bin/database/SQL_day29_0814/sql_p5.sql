...
SELECT 제품명, 카테고리명 FROM product, pcategory where product.카테고리번호_fk = pcategory.카테고리번호_pk ;

-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. (조회결과: 2개 레코드)
SELECT 제품명, 제품가격 from product, pcategory WHERE product.카테고리번호_fk = pcategory.카테고리번호_pk and pcategory.카테고리명 = "노트북";
...
SELECT 재고등록날짜, 재고수량 from product INNER JOIN stock on product.제품번호_pk = stock.제품번호_fk WHERE product.제품명 = "그램 15인치";

-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인) (조회결과: 8개 레코드)
select 제품명,카테고리명,재고수량 from pcategory INNER JOIN product on product.카테고리번호_fk = pcategory.카테고리번호_pk
...
SELECT 카테고리명,제품명 from pcategory LEFT OUTER join product on product.카테고리번호_fk = pcategory.카테고리번호_pk ;

-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요. (조회결과: 1개 레코드)
select 제품명 from product LEFT OUTER JOIN stock on product.제품번호_pk = stock.제품번호_fk where stock.제품번호_fk is null;

-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드)
select 카테고리명, sum(재고수량) 총재고수량 from pcategory inner join product on pcategory.카테고리번호_pk = product.카테고리번호_fk
    inner join stock on product.제품번호_pk = stock.제품번호_fk GROUP BY 카테고리명;

-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 
SELECT 제품명, sum(재고수량) 총재고수량 FROM product INNER JOIN stock on product.제품번호_pk = stock.제품번호_fk GROUP BY 제품명 ORDER BY 총재고수량 DESC;